...
begin
  ...
  for v := 0 to N - 1 do
    ...
    begin
      ...
      sqr1 := sqr(a1);
      sqr2 := sqr(a2);
      valuef[v, u] :=Ln(1+sqrt(sqr1 + sqr2));
  ...
  //// این سطر ها را من اضافه کردم.
   matrix.Colr:=False;
    matrix.Pixels:=N;
    matrix.Lines:=N;
    SetLength(matrix.Picture,N,N);
  ...
  Image2.Picture.Bitmap.Assign(bi);

//  matrix := ImagePr1.Are2ToGrayBitmat(anglef);
//  ImagePr1.Mat2Image(matrix, bi);
//  Image2.Picture.Bitmap.Assign(bi);
