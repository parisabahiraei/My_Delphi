...
var
  Form1: TForm1;
  matrix, matrixx, matrixxx, matrixgx, matrixgy, matrixxxx: TBitMat;
  bit: TBitmap;
...
begin
  ...
  matrixxxx.Lines := matrix.Lines;
  matrixxxx.Pixels := matrix.Pixels;

  matrixgx.Lines := matrix.Lines;
  matrixgx.Pixels := matrix.Pixels;

  matrixgy.Lines := matrix.Lines;
  matrixgy.Pixels := matrix.Pixels;
  ImagePr1.MakeBW(matrix);
  matrix.colr := False;
  ...
  setlength(GX, matrix.Lines, matrix.Pixels);
  setlength(GY, matrix.Lines, matrix.Pixels);
  setlength(matrixxxx.Picture, matrix.Lines, matrix.Pixels);

  // setlength(matrixgx.Picture, matrix.Lines, matrix.Pixels);
  // setlength(matrixgy.Picture, matrix.Lines, matrix.Pixels);
  // setlength(matrixxxx.Picture, matrix.Lines, matrix.Pixels);
  ...
  for I := 1 to matrix.Pixels - 2 do
    ...
    begin
      ...
      for x := -1 to 1 do
        for y := -1 to 1 do
        begin
          // matrixgx.Picture[j, I] := matrixgx.Picture[j, I] + MX[index] *
          // matrix.Picture[j + x, I + y];
          // matrixgy.Picture[j, I] := matrixgy.Picture[j, I] + MY[index] *
          // matrix.Picture[j + x, I + y];
          // index := index + 1;

          GX[j, I] := GX[j, I] + MX[index] * matrix.Picture[j + x, I + y];

          GY[j, I] := GY[j, I] + MY[index] * matrix.Picture[j + x, I + y];
          index := index + 1;

        end;
//      matrixxxx.Picture[j, I] :=
//        trunc(sqrt(sqr(matrixgx.Picture[j, I]) + sqr(matrixgy.Picture[j, I])));
       matrixxxx.Picture[j, I] := trunc(sqrt(sqr(GX[j, I]) + sqr(GY[j, I])));
    end;

  matrix := BitMatAssign(matrixxxx);
  ImagePr1.Mat2Image(matrix, bit);
  Image2.Picture.Bitmap.Assign(bit);
  bit.free;
end;
